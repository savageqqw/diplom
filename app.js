...
const PRESETS = [
  ...
  {
    id:'mid-gaming',name:'MID GAMING',subtitle:'Розумний вибір за кращою ціною',tag:'MID',tagColor:'#00d4ff',price:1542,
    comps:{cpu:'cpu15',gpu:'gpu28',ram:'ram10',motherboard:'mb13',storage:'st11',psu:'psu11',case:'case9',cooling:'cool12'},
    perf:{gaming:68,streaming:55,rendering:50,productivity:62},
    use:['1080p Gaming 144fps','Базовий стримінг','Легкий монтаж','Навчання'],icon:'⚡',
    desc:'RX 7800 XT 16GB VRAM та i5-14600K — ідеальне поєднання ціна/продуктивність для 1080p/1440p у 2025 році.'
  },
  {
    id:'budget',name:'BUDGET BUILD',subtitle:'Максимум за мінімум коштів',tag:'BUDGET',tagColor:'#22c55e',price:808,
    comps:{cpu:'cpu21',gpu:'gpu39',ram:'ram19',motherboard:'mb16',storage:'st16',psu:'psu15',case:'case13',cooling:'cool16'},
    perf:{gaming:46,streaming:32,rendering:26,productivity:50},
    use:['1080p Gaming 60-100fps','Офісні задачі','Навчання','Медіа-плеєр'],icon:'💡',
    desc:'RTX 4060 8GB + i5-13400F + DDR4 — найдоступніший спосіб увійти в сучасний PC-гейминг із підтримкою DLSS 3.'
  },
];
...
const state = {
  ...
  saveBuildModal: false,
  saveBuildName: '',
  saveBuildStatus: '',   // '' | 'saving' | 'saved' | 'error'
};
...
function renderNavBar() {
  const links = [
    ...
    ...(state.authUser   ? [{key:'mybuilds', label:'💾 Мої збірки'}] : []),
  ...
  <nav class="navbar" id="navbar">
    <div class="navbar-inner">
      <div class="navbar-logo" data-action="setPage" data-page="home">
        <div class="navbar-logo-icon" style="background:none;padding:0;overflow:hidden;"><img src="icon-192.png" alt="PCForge" style="width:100%;height:100%;object-fit:cover;border-radius:8px;display:block;" /></div>
        <div>
  ...
  const summaryModal = state.showSummary ? `
    <div class="modal-overlay" id="summaryOverlay">
      <div class="modal-box" id="summaryBox" style="max-width:580px">
        ...
        <!-- Export buttons -->
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:${state.authUser ? '10px' : '20px'}">
        ...
        ${state.authUser ? `
        <!-- Save to account -->
        <button data-action="openSaveBuild"
          style="display:flex;align-items:center;justify-content:center;gap:8px;width:100%;background:linear-gradient(135deg,rgba(34,197,94,.15),rgba(16,185,129,.1));border:1px solid rgba(34,197,94,.4);color:#22c55e;padding:12px 16px;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;font-family:'Exo 2',sans-serif;transition:all .2s;margin-bottom:20px;letter-spacing:.03em">
          💾 Зберегти до мого акаунта
        </button>` : `
        <div style="background:rgba(0,229,255,.05);border:1px solid rgba(0,229,255,.15);border-radius:10px;padding:12px 16px;margin-bottom:20px;font-size:12px;color:#64748b;text-align:center">
          <button data-action="openAuth" data-modal="login" style="background:none;border:none;color:#00e5ff;cursor:pointer;font-size:12px;font-family:'Exo 2',sans-serif;font-weight:700">Увійдіть</button> або
          <button data-action="openAuth" data-modal="register" style="background:none;border:none;color:#00e5ff;cursor:pointer;font-size:12px;font-family:'Exo 2',sans-serif;font-weight:700">зареєструйтесь</button>, щоб зберегти збірку до акаунта
        </div>`}
...
// ── SAVE BUILD MODAL ──────────────────────────────────────────────────────────
function renderSaveBuildModal() {
  if (!state.saveBuildModal) return '';
  const price = calcPrice(state.selected);
  const perf  = calcPerf(state.selected);
  const selectedCount = Object.values(state.selected).filter(Boolean).length;
  const isSaving = state.saveBuildStatus === 'saving';
  const isSaved  = state.saveBuildStatus === 'saved';
  const isError  = state.saveBuildStatus === 'error';

  return `
  <div class="modal-overlay" id="saveBuildOverlay" data-action="closeSaveBuildOverlay" style="z-index:3000">
    <div class="modal-box" style="max-width:460px">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px">
        <div>
          <div style="font-size:28px;margin-bottom:6px">💾</div>
          <h3 style="font-family:'Orbitron',sans-serif;font-size:18px;font-weight:900;color:#e8eaf6">Зберегти збірку</h3>
          <p style="font-size:12px;color:#64748b;margin-top:4px">${selectedCount} компонент${selectedCount===1?'':'ів'} · ${fmt(price)}</p>
        </div>
        <button data-action="closeSaveBuild" style="background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#64748b;width:32px;height:32px;border-radius:8px;font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center">✕</button>
      </div>

      ${isSaved ? `
      <div style="text-align:center;padding:32px 0">
        <div style="font-size:48px;margin-bottom:12px">✅</div>
        <div style="font-family:'Orbitron',sans-serif;font-size:16px;font-weight:800;color:#22c55e;margin-bottom:8px">Збірку збережено!</div>
        <p style="font-size:13px;color:#64748b;margin-bottom:20px">Переглянь її на сторінці «Мої збірки»</p>
        <div style="display:flex;gap:10px">
          <button data-action="closeSaveBuild" style="flex:1;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#94a3b8;padding:11px;border-radius:8px;font-size:13px;font-weight:600;cursor:pointer;font-family:'Exo 2',sans-serif">Закрити</button>
          <button data-action="goToMyBuilds" style="flex:1;background:linear-gradient(135deg,#00e5ff,#0066cc);border:none;color:#000;padding:11px;border-radius:8px;font-size:13px;font-weight:800;cursor:pointer;font-family:'Exo 2',sans-serif">Мої збірки →</button>
        </div>
      </div>` : `
      <div style="margin-bottom:20px">
        <label style="font-size:12px;color:#94a3b8;letter-spacing:.06em;font-family:'Orbitron',sans-serif;display:block;margin-bottom:8px">НАЗВА ЗБІРКИ</label>
        <input id="saveBuildNameInput" type="text" maxlength="60"
          placeholder="Наприклад: Ігровий монстр 2024, Офісна робоча станція…"
          value="${state.saveBuildName.replace(/"/g,'&quot;')}"
          style="width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(0,229,255,.3);border-radius:10px;padding:13px 16px;font-size:14px;color:#e8eaf6;font-family:'Exo 2',sans-serif;outline:none;transition:border-color .2s" />
        <div id="saveBuildCounter" style="font-size:11px;color:#334155;margin-top:6px;text-align:right">${state.saveBuildName.length}/60</div>
      </div>

      <!-- Build preview -->
      <div style="background:rgba(255,255,255,.02);border:1px solid rgba(255,255,255,.06);border-radius:10px;padding:14px;margin-bottom:20px">
        <div style="font-size:10px;color:#334155;letter-spacing:.12em;font-family:'Orbitron',sans-serif;margin-bottom:10px">КОМПОНЕНТИ</div>
        ${STEPS.map(s => {
          const item = state.selected[s] ? findComp(s, state.selected[s]) : null;
          if (!item) return '';
          const d = CD[s];
          return `<div style="display:flex;justify-content:space-between;align-items:center;padding:5px 0;border-bottom:1px solid rgba(255,255,255,.03)">
            <div style="display:flex;align-items:center;gap:6px">
              ...
              <span style="font-size:11px;color:#64748b">${item.name.split(' ').slice(0,4).join(' ')}</span>
            </div>
            <span style="font-size:11px;color:#475569;font-weight:600">${fmt(item.price)}</span>
          </div>`;
        }).join('')}
        <div style="display:flex;justify-content:space-between;align-items:center;padding-top:10px;margin-top:4px">
          <span style="font-size:12px;color:#64748b">Загальна вартість</span>
          <span style="font-family:'Orbitron',sans-serif;font-size:16px;font-weight:900;color:#00e5ff">${fmt(price)}</span>
        </div>
      </div>

      ${isError ? `<div style="background:rgba(239,68,68,.1);border:1px solid rgba(239,68,68,.25);border-radius:8px;padding:10px 14px;margin-bottom:16px;font-size:12px;color:#fca5a5">❌ Помилка збереження. Спробуйте ще раз.</div>` : ''}

      <div style="display:flex;gap:10px">
        <button data-action="closeSaveBuild"
          style="flex:1;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:#94a3b8;padding:12px;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;font-family:'Exo 2',sans-serif">
          Скасувати
        </button>
        <button data-action="submitSaveBuild" id="saveBuildBtn" ${isSaving || !state.saveBuildName.trim() ? 'disabled' : ''}
          style="flex:2;background:${state.saveBuildName.trim() && !isSaving ? 'linear-gradient(135deg,#22c55e,#166534)' : 'rgba(34,197,94,.12)'};border:${state.saveBuildName.trim() ? '1px solid rgba(34,197,94,.4)' : '1px solid rgba(34,197,94,.15)'};color:${state.saveBuildName.trim() && !isSaving ? '#fff' : '#334155'};padding:12px;border-radius:10px;font-size:14px;font-weight:800;cursor:${state.saveBuildName.trim() && !isSaving ? 'pointer' : 'not-allowed'};font-family:'Exo 2',sans-serif;transition:all .25s">
          ${isSaving ? '⏳ Збереження…' : '💾 Зберегти збірку'}
        </button>
      </div>`}
    </div>
  </div>`;
...
// ── MY BUILDS PAGE ────────────────────────────────────────────────────────────
function renderMyBuildsPage() {
  if (!state.authUser) {
    return `<div style="max-width:520px;margin:80px auto;text-align:center;padding:24px">
      <div style="font-size:56px;margin-bottom:16px">🔒</div>
      <h2 style="font-family:'Orbitron',sans-serif;font-size:22px;font-weight:900;color:#e8eaf6;margin-bottom:12px">Потрібна авторизація</h2>
      <p style="color:#64748b;font-size:14px;margin-bottom:24px">Увійдіть до акаунта, щоб переглядати та керувати своїми збірками</p>
      <div style="display:flex;gap:10px;justify-content:center">
        <button data-action="openAuth" data-modal="login" style="background:rgba(0,229,255,.12);border:1px solid rgba(0,229,255,.35);color:#00e5ff;padding:11px 28px;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;font-family:'Exo 2',sans-serif">Вхід</button>
        <button data-action="openAuth" data-modal="register" style="background:linear-gradient(135deg,#00e5ff,#0066cc);border:none;color:#000;padding:11px 28px;border-radius:10px;font-size:14px;font-weight:800;cursor:pointer;font-family:'Exo 2',sans-serif">Реєстрація</button>
      </div>
    </div>`;
  }

  const builds = state.myBuilds;
  const isLoading = !state.myBuildsLoaded;
  ...
  <div style="max-width:1100px;margin:0 auto;padding:48px 24px;position:relative;z-index:1">
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:40px;flex-wrap:wrap;gap:16px">
      <div>
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
          <span style="font-family:'Orbitron',sans-serif;font-size:10px;color:#00e5ff;letter-spacing:.2em;background:rgba(0,229,255,.1);border:1px solid rgba(0,229,255,.2);border-radius:4px;padding:3px 10px">МОЇ ЗБІРКИ</span>
        </div>
        <h1 style="font-family:'Orbitron',sans-serif;font-size:clamp(22px,4vw,38px);font-weight:900;color:#e8eaf6;margin-bottom:6px">Збережені конфігурації</h1>
        <p style="color:#64748b;font-size:14px">Акаунт: <span style="color:#00e5ff">${state.authUser.name}</span> · ${state.authUser.email}</p>
      </div>
      <button data-action="setPage" data-page="configurator"
        style="background:linear-gradient(135deg,#00e5ff,#0066cc);border:none;color:#000;padding:12px 24px;border-radius:10px;font-size:14px;font-weight:800;cursor:pointer;font-family:'Exo 2',sans-serif;letter-spacing:.03em;box-shadow:0 4px 20px rgba(0,229,255,.3);white-space:nowrap">
        + Нова збірка
      </button>
    </div>
    ...
    <div style="text-align:center;padding:60px 0;color:#334155">
      <div style="font-size:32px;animation:float 1.5s ease-in-out infinite;margin-bottom:12px">⏳</div>
      <div style="font-family:'Orbitron',sans-serif;font-size:13px;letter-spacing:.1em">Завантаження збірок…</div>
    </div>` : builds.length === 0 ? `
    <!-- Empty state -->
    <div style="text-align:center;padding:80px 24px;background:rgba(255,255,255,.02);border:1px dashed rgba(255,255,255,.08);border-radius:20px">
      <div style="font-size:64px;margin-bottom:16px;opacity:.5">🖥️</div>
      <h3 style="font-family:'Orbitron',sans-serif;font-size:18px;font-weight:700;color:#475569;margin-bottom:10px">Поки немає збірок</h3>
      <p style="color:#334155;font-size:13px;max-width:360px;margin:0 auto 24px;line-height:1.7">Зберіть конфігурацію у конфігураторі та натисни «💾 Зберегти до мого акаунта»</p>
      <button data-action="setPage" data-page="configurator"
        style="background:linear-gradient(135deg,#00e5ff,#0066cc);border:none;color:#000;padding:12px 32px;border-radius:10px;font-size:14px;font-weight:800;cursor:pointer;font-family:'Exo 2',sans-serif;box-shadow:0 4px 20px rgba(0,229,255,.25)">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:20px">
      ${builds.map((build, i) => {
        const compsArr = Object.entries(build.selected || {}).filter(([,id]) => id);
        const date = new Date(build.createdAt).toLocaleDateString('uk-UA', {day:'numeric',month:'long',year:'numeric'});
        const time = new Date(build.createdAt).toLocaleTimeString('uk-UA', {hour:'2-digit',minute:'2-digit'});
        return `
        <div style="background:rgba(255,255,255,.03);border:1px solid rgba(0,229,255,.12);border-radius:18px;overflow:hidden;animation:fadeUp .45s ease ${i*.07}s both;position:relative">
          <!-- Top accent line -->
          <div style="position:absolute;top:0;left:0;right:0;height:2px;background:linear-gradient(90deg,#00e5ff,#7c3aed)"></div>

          <!-- Card header -->
          <div style="padding:20px 20px 16px;border-bottom:1px solid rgba(255,255,255,.05)">
            <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px">
              <div style="flex:1;min-width:0">
                <h3 style="font-family:'Orbitron',sans-serif;font-size:15px;font-weight:800;color:#e8eaf6;margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis" title="${build.name.replace(/"/g,'&quot;')}">${build.name}</h3>
                <div style="font-size:11px;color:#475569">${date} · ${time}</div>
              </div>
              <button data-action="deleteSavedBuild" data-id="${build.id}" data-name="${build.name.replace(/"/g,'&quot;')}"
                style="background:rgba(239,68,68,.08);border:1px solid rgba(239,68,68,.2);color:#f87171;width:30px;height:30px;border-radius:7px;font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all .2s"
                title="Видалити">🗑</button>
            </div>

            <!-- Stats row -->
            <div style="display:flex;gap:12px;margin-top:12px">
              <div style="background:rgba(0,229,255,.07);border:1px solid rgba(0,229,255,.15);border-radius:8px;padding:8px 12px;flex:1;text-align:center">
                <div style="font-family:'Orbitron',sans-serif;font-size:15px;font-weight:900;color:#00e5ff">${fmt(build.price)}</div>
                <div style="font-size:9px;color:#475569;letter-spacing:.08em;margin-top:2px">ВАРТІСТЬ</div>
              </div>
              ${build.perf > 0 ? `<div style="background:rgba(124,58,237,.07);border:1px solid rgba(124,58,237,.15);border-radius:8px;padding:8px 12px;text-align:center;min-width:64px">
                <div style="font-family:'Orbitron',sans-serif;font-size:15px;font-weight:900;color:#a855f7">${build.perf}</div>
                <div style="font-size:9px;color:#475569;letter-spacing:.08em;margin-top:2px">ПЕРФ</div>
              </div>` : ''}
              <div style="background:rgba(34,197,94,.07);border:1px solid rgba(34,197,94,.15);border-radius:8px;padding:8px 12px;text-align:center;min-width:64px">
                <div style="font-family:'Orbitron',sans-serif;font-size:15px;font-weight:900;color:#22c55e">${compsArr.length}</div>
                <div style="font-size:9px;color:#475569;letter-spacing:.08em;margin-top:2px">КОМПОН.</div>
              </div>
            </div>
          </div>
          ...
          <div style="padding:14px 20px">
            <div style="font-size:10px;color:#334155;letter-spacing:.1em;font-family:'Orbitron',sans-serif;margin-bottom:10px">КОМПОНЕНТИ</div>
            ${STEPS.map(s => {
              const id = build.selected?.[s];
              if (!id) return '';
              const item = findComp(s, id);
              const d = CD[s];
              if (!item) return `<div style="display:flex;align-items:center;gap:6px;padding:4px 0;opacity:.4">
                <span style="font-size:12px">${d?.icon||'?'}</span>
                <span style="font-size:11px;color:#475569">Компонент не знайдено</span>
              </div>`;
              return `<div style="display:flex;align-items:center;justify-content:space-between;padding:4px 0;border-bottom:1px solid rgba(255,255,255,.03)">
                <div style="display:flex;align-items:center;gap:6px;flex:1;min-width:0">
                  <span style="font-size:12px">${d.icon}</span>
                  <div style="min-width:0">
                    <div style="font-size:9px;color:${d.accent};font-family:'Orbitron',sans-serif;letter-spacing:.05em">${d.short}</div>
                    <div style="font-size:11px;color:#94a3b8;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:200px">${item.name}</div>
                  </div>
                </div>
                <div style="font-size:11px;font-weight:700;color:#475569;flex-shrink:0;margin-left:8px">${fmt(item.price)}</div>
              </div>`;
            }).join('')}
          </div>

          <!-- Actions -->
          <div style="padding:14px 20px;border-top:1px solid rgba(255,255,255,.05);display:flex;gap:8px">
            <button data-action="loadSavedBuild" data-id="${build.id}"
              style="flex:1;background:linear-gradient(135deg,rgba(0,229,255,.15),rgba(0,229,255,.08));border:1px solid rgba(0,229,255,.3);color:#00e5ff;padding:9px;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;font-family:'Exo 2',sans-serif;transition:all .2s">
              🔧 Завантажити
            </button>
          </div>
        </div>`;
...
function render() {
  ...
  if (state.page === 'mybuilds')     pageHTML  = renderMyBuildsPage();

  app.innerHTML = renderNavBar() + pageHTML + (state.saveBuildModal && state.page !== 'mybuilds' ? renderSaveBuildModal() : '');
...
function attachEvents() {
  ...
  const saveBuildInput = document.getElementById('saveBuildNameInput');
  if (saveBuildInput) {
    saveBuildInput.addEventListener('input', e => {
      state.saveBuildName = e.target.value;
      // Update counter and button state without full re-render
      const counter = document.getElementById('saveBuildCounter');
      ...
      if (saveBtn) {
        ...
        saveBtn.style.background = hasName ? 'linear-gradient(135deg,#22c55e,#166534)' : 'rgba(34,197,94,.12)';
        saveBtn.style.color = hasName ? '#fff' : '#334155';
        saveBtn.style.cursor = hasName ? 'pointer' : 'not-allowed';
        saveBtn.style.borderColor = hasName ? 'rgba(34,197,94,.4)' : 'rgba(34,197,94,.15)';
      }
    });
    saveBuildInput.focus();
  }
  ...
  const saveBuildOverlay = document.getElementById('saveBuildOverlay');
  if (saveBuildOverlay) {
    saveBuildOverlay.addEventListener('click', e => {
      if (e.target === saveBuildOverlay) { state.saveBuildModal = false; state.saveBuildStatus = ''; render(); }
    });
  }
}
...
async function handleClick(e) {
  ...
  switch (action) {
    ...
    case 'setPage':
      ...
      // Load builds when navigating to mybuilds
      if (state.page === 'mybuilds' && state.authUser && !state.myBuildsLoaded) {
        render();
        apiFetchBuilds(state.authUser.email).then(builds => {
          state.myBuilds = builds;
          state.myBuildsLoaded = true;
          render();
        });
        break;
      }
    ...
    case 'openSaveBuild':
      state.saveBuildModal  = true;
      state.saveBuildName   = '';
      state.saveBuildStatus = '';
      render();
      setTimeout(() => document.getElementById('saveBuildNameInput')?.focus(), 80);
      break;

    case 'closeSaveBuild':
    case 'closeSaveBuildOverlay':
      if (action === 'closeSaveBuildOverlay' && e.target.id !== 'saveBuildOverlay') {
        document.addEventListener('click', handleClick, { once: true }); break;
      }
      state.saveBuildModal  = false;
      state.saveBuildStatus = '';
      render();
      break;

    case 'submitSaveBuild': {
      const buildName = (document.getElementById('saveBuildNameInput')?.value || state.saveBuildName || '').trim();
      if (!buildName) { document.addEventListener('click', handleClick, { once: true }); break; }
      if (!state.authUser) { state.authError = 'Потрібна авторизація'; render(); break; }

      state.saveBuildName   = buildName;
      state.saveBuildStatus = 'saving';
      render();

      const price = calcPrice(state.selected);
      const perf  = calcPerf(state.selected);
      const result = await apiSaveBuild(state.authUser.email, buildName, { ...state.selected }, price, perf);

      if (result.ok) {
        state.saveBuildStatus = 'saved';
        // Update local builds cache
        if (!state.myBuilds) state.myBuilds = [];
        state.myBuilds.unshift(result.build);
        state.myBuildsLoaded = true;
        render();
      } else {
        state.saveBuildStatus = 'error';
        render();
      }
      break;
    }
    ...
    case 'deleteSavedBuild': {
      const delId   = el.dataset.id;
      const delName = el.dataset.name;
      if (!confirm(`Видалити збірку «${delName}»?\nЦю дію не можна скасувати.`)) {
        document.addEventListener('click', handleClick, { once: true }); break;
      }
      el.textContent = '⏳';
      el.disabled = true;
      await apiDeleteBuild(state.authUser.email, delId);
      state.myBuilds = state.myBuilds.filter(b => b.id !== delId);
      render();
      break;
    }

    case 'loadSavedBuild': {
      const loadId = el.dataset.id;
      const found  = state.myBuilds.find(b => b.id === loadId);
      if (found && found.selected) {
        state.selected = { cpu:null,gpu:null,ram:null,motherboard:null,storage:null,psu:null,case:null,cooling:null, ...found.selected };
        state.page = 'configurator';
        state.step = 0;
        render();
      }
    ...
    case 'logout':
      ...
      state.myBuilds = [];
      state.myBuildsLoaded = false;
      if (state.page === 'mybuilds' || state.page === 'admin') state.page = 'home';
      render();
      break;
...
// ── BUILDS DB ─────────────────────────────────────────────────────────────────
const BUILDS_KEY = email => `pcforge_builds_${email}`;

const LocalBuildsDB = {
  getAll(email)  { try { return JSON.parse(localStorage.getItem(BUILDS_KEY(email)) || '[]'); } catch { return []; } },
  saveAll(email, b) { localStorage.setItem(BUILDS_KEY(email), JSON.stringify(b)); },
  add(email, build) {
  ...
  remove(email, id) {
    const list = this.getAll(email).filter(b => b.id !== id);
    this.saveAll(email, list);
  },
};

async function apiFetchBuilds(email) {
  try {
    const res = await fetch(`${API}/builds?email=${encodeURIComponent(email)}`, { signal: AbortSignal.timeout(2000) });
    if (res.ok) { const d = await res.json(); return d.builds || []; }
  } catch { /* fallback */ }
  return LocalBuildsDB.getAll(email);
}

async function apiSaveBuild(email, name, selected, price, perf) {
  const build = {
  ...
  try {
    const res = await fetch(`${API}/builds`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email, name, selected, price, perf }),
      signal: AbortSignal.timeout(3000),
    });
    if (res.ok) { const d = await res.json(); if (d.ok) return { ok: true, build: d.build }; }
  } catch { /* fallback */ }
  // Fallback: localStorage
  LocalBuildsDB.add(email, build);
  return { ok: true, build };
}

async function apiDeleteBuild(email, id) {
  try {
    const res = await fetch(`${API}/builds`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
